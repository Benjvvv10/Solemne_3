# -------------------------------------------
# Vistas principales del sistema RedBox
# - Manejo de pacientes
# - Gestión de reservas
...
from django.core.exceptions import Vagit idationError
...
def reserva_create(request):
    if request.method == 'POST':
        ...
        form = ReservaForm()
    return render(request, 'appointments/reserva_form.html', {'form': form})
...
# Vista de Login de Especialistas (prototipo básico)
def login_view(request):
    if request.method == 'POST':
        username = request.POST.get('username')
        password = request.POST.get('password')
        # Aquí se puede validar el usuario real usando authenticate si se implementa
        if username == 'doctor' and password == '1234':
            return HttpResponse('Bienvenido, especialista')
        else:
